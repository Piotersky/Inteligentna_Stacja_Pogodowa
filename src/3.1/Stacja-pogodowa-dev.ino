// Smart WiFi weather station code ver. 3.1
// By Piotersky 2024
// Owner of this code is Gaj Electronics. Made with ❤️ by Piotersky. You can't copy, send, modify or anywhere use this code without permission!
// Właścicielem tego kodu jest Gaj Electronics. Zrobione z ❤️ przez Piotersky. Nie możesz kopiować, wysyłać, modyfikować lub gdziekolwiek używać tego kodu bez zezwolenia!
...
char specialKeyLabel[31][4] = { "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "!", "@", "#", "$", "^", "&", "*", "(", ")", "-", ",", "=", "+", ":", ";", "'", "?", "/", ".", "_", "abc" };
...
const String daysOfTheWeekConjugationed[12] = { "", "poniedzialek", "wtorek", "srode", "czwartek", "piatek", "sobote", "niedziele", "poniedzialek", "wtorek", "srode", "czwartek" };
...
void error(String desc) {
  ...
  tft.drawString("Wystapil blad", 0, 0, 4);
  tft.drawString("Opis:", 0, 50, 4);
  tft.drawString(desc, 0, 100, 4);
...
void saveEEPROM() {
    ...
    EEPROM.writeShort(ssidStr.length() + passwordStr.length() + lat.length() + lon.length() + 2 + serialNumber.length() + 6, invertColours);
    EEPROM.commit();
}
...
String httpGETRequest(const char *serverName) {
  ...
  if (httpCode > 0) {
    ...
    if(apiLink.c_str() == serverName || changing) {
      return "";
    }
    error("HTTP_REQ" + String(httpCode));
    return "";
  }
...
void getApi() {
  apiLink = "https://gajelectronics.onrender.com/" + serialNumber + "/" + type + "/" + firmwareVersion + "/" + lat + "/" + lon + "/" + timezone;

  DynamicJsonDocument doc(300);
  String httpPayload = httpGETRequest(apiLink.c_str());
  ...
  if (err) {
    ...
    String ffg = "https://worldtimeapi.org/api/ip";
    
    DynamicJsonDocument doc1(300);
    String httpPayload1 = httpGETRequest(ffg.c_str());
    DeserializationError err1 = deserializeJson(doc1, httpPayload1);

    Date = doc1["datetime"].as<String>().substring(0, 10);
    Time = doc1["datetime"].as<String>().substring(11, 16);
    Hour = doc1["datetime"].as<String>().substring(11, 13).toInt();
    WDay = doc1["day_of_week"];

    return;
  }

  if (doc["Status"] == 9) {  // Error
    error("SRV_STAT");
    return;
  }
...
void getLocation() {
  lat = httpGETRequest("https://ipapi.co/latitude");
  lon = httpGETRequest("https://ipapi.co/longitude");

  saveEEPROM();
}

void drawDateTime() {
  tft.drawString(Time, 95, 0, 4);
  tft.drawString(Date, 40, 45, 4);
...
void changeSettings() {
  ...
  if (changing) {
    ...
    if (pressed != -1) {
      ...
      for (uint8_t b = 0; b < 31; b++) {
        if (key[b].justPressed()) {
          ...
          if (special) {
            if ((value == 0 || value == 1) && (b > -1 && b < 30) || ((value == 2 || value == 3) && ((b < 10 && b > -1) || b == 28))) {
    ...
    if (value == 2) { // special only
      tft.drawString("szerokosc geo. :", 120, 0, 4);
      tft.drawString(changingValue, 0, 50, 4);
      special = true;
    }
    if (value == 3) { // special only
...
void setup(void) {
  ...
  invertColours = EEPROM.readShort(ssidStr.length() + passwordStr.length() + lat.length() + lon.length() + 2 + serialNumber.length() + 6);

  if(serialNumber == "3" || serialNumber == "6" || invertColours == 1) {
...
void loop() {
  ...
  if ((millis() - lastTimeLoop) > timerDelayLoop && screen != 2) {
    if (screen != 1) { lastTimeLoop = millis(); };
    getApi();
    apiReqeuest = true;
  }

  if (screen == 1 && !changing && !isError) {
    ...
    if ((millis() - lastTimeLoop) > timerDelayLoop || apiReqeuest || clicked == true && screen == 1) {
      ...
      if (WiFi.status() == WL_CONNECTED) {
        ...
        if (day == 0) {
          ...
          tft.drawString(daysOfTheWeekConjugationed[WDay + day], 10, 50, 4);
        }
        drawForecast();
      } else {
        tft.pushImage(1, 1, 48, 48, settings_icon);
        tft.setTextSize(2);
        tft.drawString("Sprobuj ponownie", 0, 150, 4);
  ...
  if (screen == 2) {
    ...
    if (clicked && !changedScreen && screen == 2) {
      ...
      if (y_click > 195 && y_click < 230 && !changing) { getLocation(); }

      if (y_click > 325 && y_click < 355 && !changing && invertColours == 1) { invertColours = -1; inverted_chan = true; }
      if (y_click > 325 && y_click < 355 && !changing && invertColours == -1 && !inverted_chan) { invertColours = 1; }
      inverted_chan = false;

      if (y_click > 390 && y_click < 420 && !changing) {
        if (timezone < 12) {
          timezone += 1;
        } else {
          timezone = -12;
    ...
    if ((millis() - lastTimeLoop) > timerDelayLoop && !changing || clicked && screen == 2 && !changing) {
      ...
      tft.drawString("lokalizacja", 0, 195, 4);
      tft.drawString("A", 280, 195, 4);
      tft.drawString("szero.:", 0, 240, 4);
      tft.drawString("dlugo.:", 0, 280, 4);

      tft.drawString("odwroc kol.:", 0, 330, 4);
      ...
      tft.drawString(String(invertColours), 255, 330, 4);

      tft.setTextSize(1);
      tft.drawString(ssidStr, 170, 115, 4);
      tft.drawString(passwordStr, 160, 155, 4);
      tft.drawString(lat, 190, 255, 4);
      tft.drawString(lon, 190, 295, 4);
      tft.drawString("Gaj Electronics 2024 " + serialNumber + "v" + firmwareVersion, 0, 455, 4);
