...
// By Piotersky 2023
// Owner of this code is Piotersky. You can't copy, send, modify or anywhere use this code without permission!
// Właścicielem tego kodu jest Piotersky. Nie możesz kopiować, wysyłać, modyfikować lub gdziekolwiek używać tego kodu bez zezwolenia!
...
const String firmwareVersion = "2.0";
...
void checkForUpdates() {
  ...
  if (client) {
    ...
    if (https.begin(*client, newestFirmwareVersionLink)) {
      delay(100);
...
void drawTime() {
  ...
  if(timeClient.getHours() + timezone >= 24) {
    h = String(timeClient.getHours() + timezone - 24);
  }
  else {
    h = String(timeClient.getHours() + timezone);
  }
  humanTime = h;

  if (timeClient.getHours() + timezone < 10) {
      humanTime = "0" + humanTime;
  }

  if (timeClient.getMinutes() > 9) {
    humanTime = humanTime + ":" + String(timeClient.getMinutes());
  } else {
    humanTime = humanTime + ":0" + String(timeClient.getMinutes());
  }

  tft.drawString(humanTime, 95, 0, 4);
}

void drawDate() {
  if (ptm->tm_mon + 1 < 10) {
    if (ptm->tm_mday < 10) date = "0" + String(ptm->tm_mday) + ".0" + String(ptm->tm_mon + 1) + "." + String(ptm->tm_year + 1900);
    else date = "" + String(ptm->tm_mday) + ".0" + String(ptm->tm_mon + 1) + "." + String(ptm->tm_year + 1900);
  } else {
    if (ptm->tm_mday < 10) date = "0" + String(ptm->tm_mday) + "." + String(ptm->tm_mon + 1) + "." + String(ptm->tm_year + 1900);
    else date = "" + String(ptm->tm_mday) + "." + String(ptm->tm_mon + 1) + "." + String(ptm->tm_year + 1900);
  }

  tft.drawString(date, 40, 45, 4);

  day_name = daysOfTheWeek[timeClient.getDay()];
  tft.drawString(day_name, 30, 85, 4);
}
...
void drawForecast() {
  ...
  if (day == 0) {
    ...
    if (forecastHour - timeClient.getHours() <= 1) {
      forecast++;
      forecastEpochTime = doc["list"][forecast]["dt"];
      forecastEpochTime += timezone * 3600;
...
void loop() {
  ...
  if (screen == 1 && !changing) {
    ...
    if ((millis() - lastTimeLoop) > timerDelayLoop || clicked == true && screen == 1) {
      changedScreen = false;
      if (millis() > 86400) ESP.restart();
      tft.fillScreen(TFT_BLACK);
      if (WiFi.status() == WL_CONNECTED) {
        ...
        if (day == 0) {
          ...
          tft.drawString(daysOfTheWeekConjugationed[timeClient.getDay() + day - 1], 20, 50, 4);
        }
        drawForecast();
      } else {
        tft.pushImage(1, 1, 48, 48, settings_icon);
  ...
  if (screen == 2) {
    ...
    if ((millis() - lastTimeLoop) > timerDelayLoop && !changing || clicked && screen == 2 && !changing) {
      ...
      tft.drawString("By Piotersky 2023 " + serialNumber + "v" + firmwareVersion, 0, 455, 4);
